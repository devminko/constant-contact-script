...
/*
  ...
  - Cron job currently scheduled to run every: __3__ hours
*/

// GLOBAL VARIABLES
const URL = "https://api.constantcontact.com/v2";
const CURRENT_CONTACTS_LIST_ID = "1434673743";
  ...
  */
  await contacts.results.map((contact) => {
    if (contact.lists.length > 1) {
      if (contact.lists.some((list) => list.id === CURRENT_CONTACTS_LIST_ID))
        currents.push(contact);
      if (contact.lists.some((list) => list.id === LEADS_LIST_ID))
        leads.push(contact);
    }
  });
  ...
  */
  duplicates.map((contact) => {
    contact.email_addresses.map((email) =>
      emailAddresses.push({ email_addresses: [email.email_address] })
    );
  });
  ...
  if (emailAddresses.length > 0) {
    ...
    console.log(
      `${emailAddresses.length} duplicate contact(s) deleted on ${new Date()}`
    );
  }
};
...
const fetchAllContacts = async () => {
  ...
  const response = await fetch(
    `${URL}/contacts?api_key=${process.env.API_KEY}`,
    opts
  );
...
const removeContactsEndpoints = async (emailsArray) => {
  ...
  const response = await fetch(
    `${URL}/activities/removefromlists?api_key=${process.env.API_KEY}`,
    opts
  );
